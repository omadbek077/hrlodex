...
import { Job, InterviewSession, CandidateApplication, ChatMessage, Language } from '../../types';
...
const HRAnalytics: React.FC<AnalyticsProps> = ({ jobs, sessions: allSessions, applications: allApplications, messages, onSendMessage, language }) => {
  const navigate = useNavigate();
  const [selectedJobId, setSelectedJobId] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [sessionsByJob, setSessionsByJob] = useState<InterviewSession[]>([]);
  const [applicationsByJob, setApplicationsByJob] = useState<CandidateApplication[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      sessionsService.getSessionsByJob(selectedJobId),
      applicationsService.getApplicationsByJob(selectedJobId),
    ])
      .then(([sessions, applications]) => {
        setSessionsByJob(sessions);
        setApplicationsByJob(applications);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [selectedJobId]);

  const getSessionForApp = (appId: string) => sessionsByJob.find((s) => s.applicationId === appId);

  const appProfiles: ProfileRow[] = applicationsByJob.map((app) => ({
    app,
  ...
  const orphanSessions = sessionsByJob.filter((s) => !s.applicationId);
  const sessionProfiles: ProfileRow[] = orphanSessions.map((session) => ({
    app: {
  ...
  // Umumiy statistika (barcha ishlar va intervyular bo‘yicha)
  const getSessionForAppGlobal = (appId: string) => allSessions.find((s) => s.applicationId === appId);
  const globalAppProfiles: ProfileRow[] = allApplications.map((app) => ({
    app,
    session: getSessionForAppGlobal(app.id),
    totalScore: (app.analysis?.overallScore || 0) + ((getSessionForAppGlobal(app.id)?.evaluation?.overallScore || 0) * 10),
  }));
  const globalOrphanSessions = allSessions.filter((s) => !s.applicationId);
  const globalSessionProfiles: ProfileRow[] = globalOrphanSessions.map((session) => ({
    app: {
      id: 'session-' + session.id,
      jobId: session.jobId,
      name: session.candidateName || session.candidateId,
  ...
  const globalProfiles = [...globalAppProfiles, ...globalSessionProfiles];
  const totalJobs = jobs.length;
  const totalInterviews = globalProfiles.length;
  const passedWell = globalProfiles.filter((p) => {
    const s = getCandidateScoreStatus(p.app.analysis, p.session?.evaluation);
    return s === "A'lo" || s === 'Yaxshi';
  }).length;
  const passedPoorly = globalProfiles.filter((p) => {
    const s = getCandidateScoreStatus(p.app.analysis, p.session?.evaluation);
    return s === 'Yomon';
  }).length;
  const passedAverage = globalProfiles.filter((p) => {
    const s = getCandidateScoreStatus(p.app.analysis, p.session?.evaluation);
    return s === "O'rta";
  }).length;
  ...
  }, [selectedProfileId, selectedProfile?.session?.id, selectedProfile?.session?.hasRecording]);
      ...
      <div className="flex items-center gap-4">
        {selectedJobId ? (
          <button
            ...
            className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all text-slate-600 dark:text-slate-300"
          >
            <ArrowLeft size={20} />
          </button>
        ) : null}
        <div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
            {selectedJobId ? selectedJob?.title : t.analytics}
          </h2>
          <p className="text-slate-500 dark:text-slate-400 font-medium text-sm mt-1">
            {selectedJobId
              ? t.pipelineTitle + ' — ' + selectedJob?.department
              : t.selectVacancyHint}
          </p>
        </div>
      </div>

      {/* Umumiy statistika: ishlar, intervyular, yaxshi/yomon o'tganlar */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center">
              <ClipboardList className="text-slate-600 dark:text-slate-400" size={24} />
            </div>
            <div>
              <p className="text-2xl font-black text-slate-900 dark:text-white">{totalJobs}</p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">{t.statsJobs}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-black text-slate-900 dark:text-white">{totalInterviews}</p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">{t.statsInterviews}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-900/30 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-black text-emerald-700 dark:text-emerald-400">{passedWell}</p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">{t.statsPassedWell}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-red-100 dark:border-red-900/30 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-black text-red-700 dark:text-red-400">{passedPoorly}</p>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">{t.statsPassedPoorly}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {!selectedJobId ? (
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {jobs.length === 0 ? (
              <p className="text-slate-500 dark:text-slate-400 col-span-2">{t.noVacanciesHint}</p>
            ) : (
              jobs.map((job) => (
                <button
                  key={job.id}
                  onClick={() => setSelectedJobId(job.id)}
                  className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 hover:shadow-lg hover:border-indigo-200 dark:hover:border-indigo-900 transition-all text-left group"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:scale-105 transition-transform">
                      <Briefcase size={24} />
                    </div>
                    <div>
                      <p className="text-lg font-black text-slate-900 dark:text-white">{job.title}</p>
                      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                        {job.department} • {t.inviteCodeLabel}: {job.inviteCode}
                      </p>
                    </div>
                  </div>
                  <ArrowRight size={20} className="text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all shrink-0" />
                </button>
              ))
            )}
          </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  {unifiedProfiles.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-16 text-center text-slate-500 dark:text-slate-400 font-bold">
                        {t.noCandidatesForJob}
                      </td>
                    </tr>
                  ) : (
                    unifiedProfiles.map((p) => (
                      <tr
                        ...
                        onClick={() => setSelectedProfileId(p.app.id)}
                      >
                        <td className="py-6 px-4">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-900/30 rounded-2xl flex items-center justify-center font-black text-indigo-700 dark:text-indigo-400">
                              {(getDisplayName(p) || '?')[0]}
                            </div>
                            <div>
                        ...
                        <td className="py-6 px-4 text-right">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setSelectedProfileId(p.app.id);
                            }}
                            className="p-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-600 dark:hover:border-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all text-slate-400"
                          >
                            <ArrowRight size={16} />
                          </button>
                        </td>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-6xl max-h-[95vh] overflow-hidden rounded-[4rem] shadow-2xl flex flex-col transition-colors">
            <div className="p-10 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 bg-indigo-600 rounded-[2rem] flex items-center justify-center text-white text-3xl font-black">
                  {(getDisplayName(selectedProfile) || '?')[0]}
                </div>
                <div>
                  ...
                  {(selectedProfile.session?.candidateId && selectedProfile.session.candidateId.startsWith('CAND-')) && (
                    <p className="text-slate-400 dark:text-slate-500 font-mono text-xs mt-1">
                      ID: {selectedProfile.session.candidateId}
                    </p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="p-4 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl transition-all text-slate-400 dark:text-slate-500"
                >
                  <X size={24} />
                </button>
              </div>
            ...
            <div className="flex border-b border-slate-100 dark:border-slate-800 px-10">
              {[
                ...
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as 'Analysis' | 'Interview' | 'Chat')}
            ...
            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
              ...
              {activeTab === 'Analysis' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                  {selectedProfile.session?.status !== 'Completed' ||
                  (!selectedProfile.app.analysis && !selectedProfile.session?.evaluation) ? (
                  ...
                  <div className="bg-orange-50 dark:bg-orange-900/10 p-8 rounded-[2.5rem] border border-orange-100 dark:border-orange-900/30">
                    <p className="text-xl font-bold text-slate-700 dark:text-slate-300 italic">
                      "{selectedProfile.app.analysis?.summary || selectedProfile.session?.evaluation?.summary || '—'}"
                    </p>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700">
                      <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-6">
                        {t.scoreMatrix}
                      </h4>
                      <div className="space-y-4">
                        {(selectedProfile.app.analysis
                          ...
                          <div key={i} className="space-y-1">
                            <div className="flex justify-between text-[10px] font-black dark:text-slate-300">
                              <span>{m.l}</span>
                      ...
                      {selectedProfile.session?.evaluation?.overallRecommendation && (
                        <p className="mt-4 text-sm font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">
                          {translateRecommendation(selectedProfile.session.evaluation.overallRecommendation)}
                        </p>
                      )}
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-700">
                      <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-6">
                        {selectedProfile.app.analysis ? t.detectedSkills : t.strengthsWeaknesses}
                      </h4>
                      {selectedProfile.app.analysis?.detectedSkills?.length ? (
                        <div className="flex flex-wrap gap-2">
                          {selectedProfile.app.analysis.detectedSkills.map((s, i) => (
                            <span
                              key={i}
                              className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-bold text-indigo-600 dark:text-indigo-400"
                            >
                              {s}
                            </span>
                          ))}
                        ...
                        <div className="space-y-4">
                          {selectedProfile.session.evaluation.strengths?.length ? (
                            <div>
                              ...
                              <ul className="space-y-1 text-sm text-slate-700 dark:text-slate-300">
                                {selectedProfile.session.evaluation.strengths.map((s, i) => (
                                  <li key={i}>• {s}</li>
                                ))}
                              </ul>
                            </div>
                          ...
                          {selectedProfile.session.evaluation.weaknesses?.length ? (
                            <div>
                              <p className="text-[9px] font-black text-orange-600 dark:text-orange-400 uppercase tracking-widest mb-2">
                                {t.weaknesses}
                              </p>
                              <ul className="space-y-1 text-sm text-slate-700 dark:text-slate-300">
                                {selectedProfile.session.evaluation.weaknesses.map((w, i) => (
                                  <li key={i}>• {w}</li>
                                ))}
                              </ul>
                            </div>
                    ...
                    <>
                      ...
                      <div className="bg-indigo-600 p-8 rounded-[2.5rem] text-white">
                        <p className="text-xl font-bold italic">
                          "{selectedProfile.session.evaluation?.summary}"
                        </p>
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4 p-4 custom-scrollbar">
                    {selectedProfile?.app.id?.startsWith('session-') ? (
                      <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 text-center">
                        ...
                        <p className="text-xs font-black uppercase tracking-widest max-w-[280px]">
                          Chat mavjud emas — bu to&apos;g&apos;ridan-to&apos;g&apos;ri invite orqali intervyu
                        </p>
                      </div>
            ...
            <div className="p-10 border-t border-slate-100 dark:border-slate-800 flex justify-end">
              <button
                onClick={() => setSelectedProfileId(null)}
                className="px-12 py-4 bg-slate-900 dark:bg-slate-800 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-slate-800 dark:hover:bg-slate-700 transition-all"
              >
                {t.closeDossier}
              </button>
            </div>
